...
#+BEGIN_SRC sh
rest-shell --server http://localhost:8080
#+END_SRC
...
#+BEGIN_SRC sh
rest-shell http://localhost:8080
#+END_SRC
...
- Add support for timing out on the server side, useful if you run a command that requires input
- Option to return stdout and stderr separately instead of combined in the server API
- Input via stdin is currently limited to shell pipelining
